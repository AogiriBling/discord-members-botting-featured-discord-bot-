...
LogsChannel = 947178068349431878
...
@bot.event 
async def on_ready():
  guild = bot.get_guild(896456338509545532)
  print(f'Bot Online | Bot Latency {bot.latency} | Proxy Type {TypeOfProxy}')
  await bot.change_presence(status=discord.Status.do_not_disturb, activity=discord.Activity(type=discord.ActivityType.watching, name=f"Aogiri Tree"))
  ...
  if len(queue) == 0:
    ...
    if r.json() is not None and r.json()["code"] == 0:
      ...
      await channelGet.send(embed=embed)
        ...
        if r.json()['guild'] is not None and r.json()['guild']['id'] == 896456338509545532:
          r = requests.delete(f'https://discord.com/api/v9/users/@me/guilds/896456338509545532', headers=headers)
        else:
          pass
...
async def help(ctx):
  if ctx.channel.id != 947178068349431878 and ctx.channel.type != discord.ChannelType.private:
    embed = discord.Embed(title='Commands', description='$help\n$ban\n$kick\n\n `Anti Alt: 7 Days`\n `Anti Spam: 5 Messages`',color=color)
    embed.set_thumbnail(url='https://nationalzoo.si.edu/sites/default/files/animals/exhibit/africanlion-005.jpg')
    await ctx.send(embed=embed)
    ...
    if "mod" not in ctx.author.name or "admin" not in ctx.author.name:
          BotUser = await bot.fetch_user(876509440529031208)
          embed = discord.Embed(title='Lion Commands',color=color)
          ...
          embed.set_footer(text="Developed By shees#6402 & Founded By Glue#0001", icon_url=BotUser.avatar_url)
          await ctx.send(embed=embed)
  else:
...
async def join(ctx, serverlink, amount = None):
  allowedChannels = [947178068349431878]
  if ctx.channel.id not in allowedChannels: 
    embed = discord.Embed(
      title="You can't use this command here",
      description='You must use this command in <#947178068349431878> ',
      color=color
    )
    ...
    await a.delete()
      ...
      silverRole = discord.utils.get(ctx.guild.roles,id=920769775821529119)
      BronzeRole = discord.utils.get(ctx.guild.roles,id=894258754831405108)
      GoldRole = discord.utils.get(ctx.guild.roles,id=896505681438474260)
      PremiumRole = discord.utils.get(ctx.guild.roles,id=896505962658152498)
      s = serverlink.split('/')
      if len(s) > 4:
...
async def r4id(ctx, link, channel, *, message):
  if ctx.channel.id != 947178068349431878:
    embed = discord.Embed(
      title="You can't use this command here",
      description='You must use this command in <#947178068349431878> ',
      color=color
    ...
    await asyncio.sleep(3)
    await a.delete()
  elif int(channel) == 947178068349431878 or int(channel) == 896456338509545535 or int(channel) == 896591258875736085 or int(channel) == 896788635729395753 or int(channel) == 896794926933753936:
...
async def leaveserver(ctx, id):
  if ctx.channel.id != 947178068349431878:
    ...
    silverRole = discord.utils.get(ctx.guild.roles,id=920769775821529119)
    BronzeRole = discord.utils.get(ctx.guild.roles,id=969602448576630914)
    GoldRole = discord.utils.get(ctx.guild.roles,id=958428030903136308)
    PremiumRole = discord.utils.get(ctx.guild.roles,id=920769775821529119)
...
async def on_member_join(member):
  if "/tokenz" in member.name or "Shroom" in member.name or "Ghost-dev" in member.name or "Groom" in member.name:
    await member.kick()
  else:
    pass
...
async def kalt(ctx):
  for member in ctx.guild.members:
    if "Shroom" in member.name or "Ghost-Dev" in member.name:
      await member.kick()
    else:
      pass
...
async def button(ctx): 
  headers = {
    "Authorization": "ODk2MjM3NjUzMzcwOTk0NzA4.YWFAAQ.Kb10KQ5Nyyauyl5jGv7OVWn4yew"
  }
  payload = {"type": 3,"guild_id": "896742669211017226","channel_id": "898481997222137878","message_id": "898483454994415676","application_id": "896237653370994708",'data': {'compontent_type': 2, 'custom_id': '暋ࣁ微⟺徍囈㩅繁ղ汦ℕ犦殿媧寐㻉枱槮็椕絮坪ಁ㾯糎燩㝀耀'}}
